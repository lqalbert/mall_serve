...
class CreateOrderBasicTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('order_basic', function (Blueprint $table) {
            ...
            $table->string('address', 80)->comment('收货地址')->nullable();
            $table->string('det_address', 80)->comment('详细收货地址')->nullable();
            $table->string('user_phone', 80)->comment('收件人手机')->nullable();
            $table->string('goods_name', 60)->comment('商品名称')->nullable();
            $table->string('print', 60)->comment('打印')->nullable();
            ...
            $table->string('pay_name', 60)->comment('支付方式')->nullable();
            $table->enum('shipping_status', ['0','1','2'])->comment('发货状态0待发货1已发货2已收货')->nullable();
            $table->enum('check_status', ['0', '1','2'])->comment('审核状态0未通过1通过2未审核')->default(0);
            $table->string('shipping_name', 60)->comment('配送方式')->nullable();
